    ...
    <v-main>
      <v-container fluid class="fill-height">
        <v-row no-gutters class="fill-height">
          <!-- Column left -->
          <v-col cols="12" md="6" class="d-flex flex-column justify-center align-center px-10 text-center">
            <div>
          ...
          <v-col cols="12" md="6" class="d-flex justify-center align-start" style="padding-top: 40px;">
            <v-card class="pa-12 pb-8" elevation="8" max-width="448" rounded="lg">
              <div class="text-subtitle-1 text-medium-emphasis">Cuenta</div>

              <v-text-field v-model="email" :rules="emailRules" density="compact" placeholder="Correo electrónico"
              ...
              <router-link class="text-caption text-decoration-none text-blue" to="/recuperarclave">
                ¿Olvidaste tu contraseña?
              </router-link>

              <v-card class="mb-6" color="surface-variant" variant="tonal">
                <v-card-text class="text-medium-emphasis text-caption">
                  Advertencia: Después de 3 intentos fallidos consecutivos, tu cuenta será bloqueada temporalmente por
                  tres horas.
                </v-card-text>
              </v-card>

              <v-btn class="mb-6" color="blue" size="large" variant="tonal" block @click="login" :loading="loading">
                Iniciar sesión
              </v-btn>
...
const login = async (): Promise<void> => {
  ...
  try {
    ...
    alert('Error: Credenciales incorrectas o problema con el servidor.')
  } finally {
    loading.value = false
  }
}
